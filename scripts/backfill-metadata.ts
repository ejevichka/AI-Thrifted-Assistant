...
// Fetch brands without metadata from database
...
async function backfillBrandMetadata() {
  console.log('\n🚀 Starting brand metadata backfill...\n');
  ...
  if (failedUpdates.length > 0 && failedUpdates.length < 50) {
    console.log(`\n\n🔄 Retrying ${failedUpdates.length} failed brands...\n`);

    for (const brandName of failedUpdates) {
      console.log(`Retry: ${brandName}`);

      const brand = brandsToProcess.find(b => b.entity_name === brandName);
      if (!brand) continue;

      const metadata = await generateMetadataForBrand(brandName);

      if (metadata) {
        const success = await updateBrandMetadata(brand.id, metadata);
        if (success) {
          console.log(`✅ Retry successful!`);
          successfulUpdates.push(brandName);
          failedUpdates.splice(failedUpdates.indexOf(brandName), 1);
        }
      }
...
// Run the script
backfillBrandMetadata().catch(console.error);
