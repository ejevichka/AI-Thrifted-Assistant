...
// RESUME: Set this to the last token before the 429 error
const RESUME_TOKEN = '1734194844'; // Last successful token

// Increased delay to avoid rate limiting - 3 seconds between requests
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        ...
        return fetchFavoritesPage(nextPageToken); // Retry after waiting
      }

      if (response.status === 401 || response.status === 403) {
        console.error('\n🔑 Authentication failed! Tokens expired.');
      }
...
async function fetchRemainingFavorites(): Promise<FavoriteItem[]> {
  // Load existing data
  ...
  let nextPageToken: string | undefined = RESUME_TOKEN;
  let pageNumber = 200; // Approximate page number where we left off

  console.log(`\n🔄 Resuming from token: ${RESUME_TOKEN}`);
  console.log(`⏱️  Using ${DELAY_BETWEEN_REQUESTS}ms delay between requests\n`);
  ...
  while (true) {
    ...
    console.log(`Found ${items.length} items on this page (${newItems.length} new)`);
...
async function main() {
  console.log('🔄 Resuming Vinted favorites scraper...');
  console.log(`User ID: ${USER_ID}\n`);

  const favorites = await fetchRemainingFavorites();
