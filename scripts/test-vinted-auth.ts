/**
 * Simple script to test if your Vinted authentication tokens are valid
 * Run this before fetch-vinted-favorites.ts to verify your setup
 */
...
async function testAuth() {
  console.log('🔍 Testing Vinted authentication...\n');

  console.log(`User ID: ${USER_ID}`);
  console.log(`CSRF Token: ${CSRF_TOKEN.substring(0, 20)}...`);
  console.log(`Anon ID: ${ANON_ID.substring(0, 20)}...`);
  console.log(`Cookies length: ${COOKIES.length} characters\n`);

  const url = `https://www.vinted.com/api/v2/users/${USER_ID}/items/favourites`;
  ...
  try {
    ...
    if (response.ok) {
      ...
      const hasNextPage = !!data.pagination?.next_page_token;

      console.log('\n✅ SUCCESS! Authentication works!\n');
      console.log(`Found ${itemCount} items on first page`);
      ...
      if (itemCount > 0) {
        ...
        console.log(`  Brand: ${firstItem.brand_title || firstItem.brand || 'Unknown'}`);
      }

      console.log('\n✅ You can now run: yarn fetch:favorites');
    } else {
      console.log('\n❌ FAILED! Authentication error\n');

      if (response.status === 401 || response.status === 403) {
        console.error('Your tokens are expired or invalid.');
        console.error('\nCommon issues:');
        console.error('1. Cookies expire quickly - get fresh ones');
        console.error('2. Make sure you copied the ENTIRE cookie string');
        console.error('3. CSRF token must match the current session');
        console.error('4. You must be logged in to Vinted in your browser');
        console.error('\nTry getting new tokens by:');
        console.error('- Logging out and logging back in to Vinted');
        console.error('- Clearing cookies and logging in fresh');
        console.error('- Using Incognito mode and logging in there');
      } else if (response.status === 404) {
        console.error('User ID not found. Check the USER_ID variable.');
      } else {
        console.error('Unexpected error. Check your network connection.');
      }
    ...
    console.error('2. Vinted is accessible (not blocked)');
    console.error('3. No firewall/proxy issues');
  }
}
